...
void sdp_media_laddr_rtcp(const struct sdp_media *m, struct sa *laddr)
{
    if (!m || !laddr)
        return;
    ...
    if (sa_isset(&m->laddr_rtcp, SA_ALL)) {
        ...
        *laddr = m->laddr;
        sa_set_port(laddr, sa_port(&m->laddr_rtcp));
    }
    else {
        uint16_t port = sa_port(&m->laddr);

        *laddr = m->laddr;
        sa_set_port(laddr, port ? port + 1 : 0);
    }
}
