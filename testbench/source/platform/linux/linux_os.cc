...
namespace host
{
    ...
    bool OSSock::Select(OSSock *skset[],int nr,int timeout)
    {
        ...
        for (int i = 0; i < nr; i++) 
        {
            if (!FD_ISSET((int)(skset[i]->handle), &fdread))
                skset[i]= NULL;
        }
        return true;
    }
    ...
    bool OSSock::Connect(const char* strIP,unsigned nPort)
    {
        if(handle)
        {
            ...
            if (connect((int)handle, (struct sockaddr*)&addr, sizeof(addr)) < 0)
                return false;
    ...
    bool OSSock::Bind(const char* strIP,unsigned nPort)
    {
        if(handle)
        {
            ...
            if (bind((int)handle, (struct sockaddr*)&addr, sizeof(addr)) < 0)
                return false;
    ...
    bool OSSock::Listen ()
    {
        ...
        return (listen((int)handle,SOMAXCONN)>0);
    }
    ...
    bool OSSock::Accept(OSSock *&ref_sock)
    {
        ...
        sock = accept((int)handle, (struct sockaddr*)&addr, &addrlen);
        if (sock != -1)
        {
    ...
    int  OSSock::Recv(void* lpBuf, int nBufLen,unsigned long*addIP,unsigned *nPort)
    {
        ...
        if(addIP || nPort)
        {
            ...
            retval = recvfrom((int)handle, (char*)lpBuf,nBufLen, 0,(struct sockaddr*)&addr,&size);
            if(retval>0)
            {
                if(addIP)
        ...
        {
            return recv((int)handle, (char*)lpBuf,nBufLen, 0);
        }
    }
    ...
    int  OSSock::Send(void* lpBuf, int nBufLen,unsigned long addIP,unsigned nPort)
    {
        ...
        if(addIP!=0)
        {
            ...
            return sendto((int)handle,(char*)lpBuf,nBufLen, 0,(struct sockaddr*)&addr,sizeof(addr));
        }
        else
        {
            return send((int)handle,(char*)lpBuf,nBufLen, 0);
        }
        return 0;
    }
    ...
    bool OSSock::SetBlock(bool blocking)
    {
        ...
        long flags = fcntl((int)handle,F_GETFL);
        ...
        fcntl((int)handle,F_SETFL,flags);
        return true;
    }
